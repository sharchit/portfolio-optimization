...
class Portfolio(gym.Env):
    def __init__(self, price_matrix, state_matrix, A=80000, B=0.5, TC=0.006):
        ...
        self.index = 0
        self.action_space = spaces.Box(low=np.zeros(10), high=np.ones(10))
    ...
    def step(self, action_attn):
        ...
        self.curr = self.po_value(action, (self.price_matrix[self.index] - self.price_matrix[self.index - 1]) / self.price_matrix[self.index - 1])
...
#     obs, reward, done, _, _ = env.step({'action': action, 'attn': 0.0* np.ones(64)})
#     rew.append(reward)
